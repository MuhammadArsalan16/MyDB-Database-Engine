...
static void test_write_at_explicit_offset(void)
{
    ...
    int rc = large_wal_segment_pool_write(&pool, &slot_index, &page_no, &offset, chunk1, sizeof(chunk1));
    ...
    rc = large_wal_segment_pool_write(&pool, &slot_index, &page_no, &offset, chunk2, sizeof(chunk2));
...
static void test_write_spans_multiple_pages(void)
{
    ...
    int rc = large_wal_segment_pool_write(&pool, &slot_index, &page_no, &offset, big, big_len);
...
static void test_write_rolls_to_new_segment(void)
{
    ...
    int rc = large_wal_segment_pool_write(&pool, &slot_index, &page_no, &offset, big, total_len);
