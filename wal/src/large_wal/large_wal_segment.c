 ...
 * wal_segment.c already documents: avoids alignment assumptions about
 * caller-provided buffers.
 *
 * Caller sets every field of *hdr except checksum before calling
 * large_wal_segment_header_serialize(); id.magic/id.version are
...
/* LargeWalSegmentHeader — documented on-disk shape is 48 bytes,
 * serialized into a 64-byte on-disk buffer (16 trailing bytes zeroed,
 * reserved for future extension). Checksum covers the 44 bytes
 * preceding it (id[8] + segment_no[8] + start_lsn[8] + end_lsn[8] +
 * partition_id[4] + data_pages[4] + state[1] + reserved[3]). */
#define LARGE_WAL_SEGMENT_HEADER_CHECKSUM_OFFSET 44

void large_wal_segment_header_serialize(const LargeWalSegmentHeader *hdr, uint8_t *buf)
{
    memset(buf, 0, LARGE_WAL_SEGMENT_HEADER_ON_DISK_SIZE);
    ...
    uint32_t cs = crc32(buf, LARGE_WAL_SEGMENT_HEADER_CHECKSUM_OFFSET);
    memcpy(buf + LARGE_WAL_SEGMENT_HEADER_CHECKSUM_OFFSET, &cs, 4);
}

int large_wal_segment_header_deserialize(const uint8_t *buf, LargeWalSegmentHeader *out)
{
    int rc = file_header_check_id(buf, FILETYPE_LARGE_WAL_SEGMENT);
    if (rc != MYDB_OK) return rc;

    uint32_t stored;
    memcpy(&stored, buf + LARGE_WAL_SEGMENT_HEADER_CHECKSUM_OFFSET, 4);
    if (stored != crc32(buf, LARGE_WAL_SEGMENT_HEADER_CHECKSUM_OFFSET))
